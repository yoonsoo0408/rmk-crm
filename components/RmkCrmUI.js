...
const ASSIGNEES = ["나윤수", "조맹현", "김유진", "김지훈", "전정열", "노은진"];
...
export default function RMKCrm() {
    ...
    try {
      ...
      if (json.data) {
        ...
        const mapped = json.data.map(d => ({
          ...
          companyEmail: d.company?.email || null,
          contactName: d.company?.contact_name || null,
          client: d.client?.name || d.client || "",
          stage: d.stage,
          amount: d.amount || 0,
          currency: d.currency || "USD",
          daysOverdue: d.days_overdue || 0,
          assignee: d.assignee?.name || d.assignee || null,
          assigneeId: d.assignee?.id || null,
          importance: d.importance || 3,
          destination: d.destination_port || null,
          loadingPort: d.loading_port || null,
          shipmentTerm: d.shipment_term || null,
          paymentTerm: d.payment_term || null,
          product: d.product || "",
          totalQty: d.total_qty || null,
          totalWeight: d.total_weight || null,
          firstContactDate: d.first_contact_date || null,
          quoteRef: d.quote_ref || null,
          validityEnd: d.validity_end || null,
          daysStale: d.days_stale || 0,
        }));
        setDeals(mapped);
      }
    } catch(e) {
  ...
  const updateDeal = async (id, patch) => {
    ...
    try {
      const apiPatch = {};
      if (patch.assignee !== undefined) apiPatch.assignee_id = patch.assigneeId || null;
      if (patch.stage !== undefined) apiPatch.stage = patch.stage;
      if (patch.importance !== undefined) apiPatch.importance = patch.importance;
  ...
  if (loading) return (
    <div style={{ background:"#0C0E12", minHeight:"100vh", display:"flex", alignItems:"center", justifyContent:"center" }}>
      <div style={{ textAlign:"center" }}>
        <div style={{ fontFamily:"'DM Mono',monospace", fontSize:24, fontWeight:700, color:"#F59E0B", marginBottom:12 }}>RMK/CRM</div>
        <div style={{ color:"#4B5563", fontSize:13, fontFamily:"'Noto Sans KR',sans-serif" }}>데이터 불러오는 중...</div>
      </div>
    </div>
  );
